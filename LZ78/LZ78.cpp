...
bool compressLZ78 (const char *fileName, FileInfo *myFile, const char *fileOutput) {
    ...
    std::ofstream FileOut(fileOutput + getFileName(fileName) + "." + "lz78", std::ifstream::out | std::ifstream::binary);
    ...
    std::cout << myFile->compressedChar << std::endl;
    while (FileIn.good()) {
        byte = FileIn.get();
...
bool decompressLZ78 (const char *fileName, FileInfo *myFile, const char *fileOutput) {
    ...
    std::ofstream FileOut(fileOutput + getFileName(fileName) + "." + myFile->fileExtension, std::ifstream::out | std::ifstream::binary);
    if (!FileOut.good()) return false;
